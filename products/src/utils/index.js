...
//Raise Events with improved error handling
module.exports.PublishCustomerEvent = async (payload) => {
  try {
    const response = await axios.post("http://customer:8001/app-events/", {
      payload,
    });
    console.log("Customer event published successfully:", response.status);
...
module.exports.PublishShoppingEvent = async (payload) => {
  try {
    const response = await axios.post("http://shopping:8003/app-events/", {
      payload,
    });
    console.log("Shopping event published successfully:", response.status);
    return response.data;
  } catch (error) {
...
module.exports.SubscribeMessage = async (channel, service) => {
  try {
    ...
    await channel.bindQueue(q.queue, EXCHANGE_NAME, service);
    console.log(`Queue bound to exchange ${EXCHANGE_NAME} with routing key ${service}`);
